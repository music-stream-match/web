...
Musica ("the App") is a free, open-source web application that helps users transfer playlists between streaming music services. This Privacy Policy explains what data we collect, how we use it, and your rights.
...
Deezer API requests are routed through a Cloudflare Worker proxy (`deezer-musica.mobulum.com`) to handle CORS restrictions. This proxy may log standard HTTP request metadata (IP addresses, timestamps, user agents) as part of Cloudflare's default infrastructure. We do not actively collect or store this data, but Cloudflare's infrastructure may retain logs temporarily in accordance with [Cloudflare's Privacy Policy](https://www.cloudflare.com/privacypolicy/).
